      ...
      <ul class="nav nav-tabs">
         ...
         <li class="nav-item nav-list">
            <form action="{{ route('users.followed', Auth::id()) }}" method="post">
               @csrf
               <input class="nav-link btn-link active" type="submit" value="フォロー" style="background-color:#F7FAFC; border-bottom: 1px solid #DEE2E6;">
            </form>
         </li>
         <li class="nav-item nav-list">
            ...
            <div class=" my-5">
               @foreach($followed as $follow)
                  <div class="toukou ml-3 w-75 mx-auto">
                        <!-- タイトル -->
                     <h5><a href="{{ route('users.profile',$follow->to_user->id) }}">{{$follow->to_user->name}}</a></h5>
                        <!--ユーザー名、年齢/性別-->
                     <p></p>
                     <!-- 投稿日時 -->
                     <hr>
